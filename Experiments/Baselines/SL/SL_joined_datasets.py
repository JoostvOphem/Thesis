...
if WANDB_ENABLED:
    ...
    run = wandb.init(
        ...
        name=f'SL_joined_{DATASET1}_{DATASET2}'
    )
...
val_data = np.concatenate((val_dataA, val_dataB), axis=0)
val_labels = np.concatenate((val_labelsA, val_labelsB), axis=0)
test_data = np.concatenate((test_dataA, test_dataB), axis=0)
test_labels = np.concatenate((test_labelsA, test_labelsB), axis=0)
